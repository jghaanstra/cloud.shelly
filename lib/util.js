...
class Util {
  ...
  async processDeviceChange(shellies, coapdevice, prop, newValue, oldValue) {
    try {
      const shelly = shellies.filter(obj => Object.keys(obj).some(key => obj[key].includes(coapdevice)));
      if (shelly.length > 0) {
        ...
        const parseCapability = this.coapToCapabilityParser(prop, newValue, shelly[0].driver);
        ...
        console.log('No matching Homey device found for discovered coap device', coapdevice);
        return Promise.resolve('No matching Homey device found for discovered coap device', coapdevice);
      }
    } catch (error) {
      console.log(error);
  ...
  coapToCapabilityParser(prop, value, driver) {
    ...
    } else if (prop === 'gain' || (driver === 'shellyduo' && prop === 'brightness') || (driver === 'shellyvintage' && prop === 'brightness') || (driver === 'shellyrgbw2white' && (prop === 'brightness0' || prop === 'brightness1' || prop === 'brightness2' || prop === 'brightness3'))) {
    ...
    } else if (prop === 'red' || prop === 'green' prop === 'blue' || (prop === 'brightness' && driver === 'shellybulb')) {
      // TODO: solve the problem of individual RGB reports
      // TODO: red, green, blue, white and gain determine output power when mode is set to color. Temp and brightness determine output when mode is white
    } else if (prop === 'mode') {

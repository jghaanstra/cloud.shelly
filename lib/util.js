...
class Util {
  ...
  static deviceConfig = [
    ...
    {
      'hostname': ['shellypro3em-', 'ShellyPro3EM-', 'shellypro3em-monophase-', 'ShellyPro3EM-monophase-'],
      'name': 'Shelly Pro 3EM',
      'gen': 'gen2',
    ...
    {
      'hostname': ['shellypro3em-triphase-', 'ShellyPro3EM-triphase-'],
    ...
    {
      'hostname': ['shelly0110dimg3-', 'Shelly0110DimG3-'],
      'name': 'Shelly Dimmer 0/1-10V PM',
      'gen': 'gen3',
      'type': ['S3DM-0010WW'],
      'id': 'shelly0110dimg3',
      'channels': 1,
 ...
 // SUPPORTED DEVICES
 let supported_device = ['SBBT-00', 'SBDW-00', 'SBMO-00', 'SBHT-00', 'SBBT-EU', 'SBBT-US'];
 ...
 function parseResult(ev, res) {
   ...
   if (supported_device.indexOf(res.local_name.substring(0, 7)) === -1) return;
 ...
 if (BLEConfig.enable === false) {
  ...
  filterBLEDevices(type) {
    const deviceid = type.substring(0, 7);
  ...
  getDeviceConfig(id, type = null) {
    try {
      if (type === null) {
        return Util.deviceConfig.find(r => r.id.includes(id));
      } else if (type === 'zwave') {
        return Util.deviceConfig.find(r => r.name.includes(id));
      } else if (type === 'bluetooth') {
        const deviceid = id.substring(0, 7);
        return Util.deviceConfig.find(r => r.type.includes(deviceid));
      } else {
        return Util.deviceConfig.find(r => r.type.includes(id));
      }
    } catch (error) {
      console.error(error);
    }

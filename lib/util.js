...
class Util {
  ...
  static deviceConfig = [
    ...
    },
    {
      ...
      'battery': false,
      'class': 'socket',
      'capabilities_1': ["measure_power", "meter_power_factor", "measure_current", "measure_voltage", "meter_power_returned", "meter_power", "measure_temperature", "rssi"],
      'capabilities_2': ["measure_power", "meter_power_factor", "measure_current", "measure_voltage", "meter_power_returned", "meter_power"],
      'capability_options': {"measure_temperature": {"title": {"en": "Device Temperature","nl": "Apparaat temperatuur"}}},
      'energy': {"cumulative": true},
    ...
    },
    {
      ...
      'battery': true,
      'class': 'sensor',
      'capabilities_1': ["alarm_smoke", "measure_battery", "measure_voltage", "rssi", "devicePlusSmoke"],
      'capabilities_2': [],
      'capability_options': {},
      'energy': {"batteries": ["CR123A"]},
      'triggers': [],
    ...
    },
    {
      ...
      'battery': false,
      'class': 'socket',
      'capabilities_1': ["onoff", "measure_power", "meter_power", "measure_voltage", "measure_current", "measure_temperature", "rssi"],
      'capabilities_2': [],
      'capability_options': {'measure_temperature': {"title": {"en": "Device Temperature","nl": "Apparaat temperatuur"}}},
      'energy': {},
      'triggers': [],
  ...
  async setUnicast(address, username, password) {
    try {
      ...
      if (settings.hasOwnProperty("coiot")) {
        ...
        return Promise.resolve('Device with IP address '+ address +' does not support unicast, make sure you update your Shelly to the latest firmware.');
      }
    } catch (error) {
      console.error(error);
      return Promise.reject(error);
  ...
  async setWsServer(address, password) {
    try {
      ...
      if (config.hasOwnProperty("ws")) {
        ...
        const update = this.sendRPCCommand('/rpc/Shelly.Update', address, password);

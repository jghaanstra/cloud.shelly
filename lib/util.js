...
class Util {
  ...
  async getCloudDetails() {
    try {
      ...
      return Promise.resolve({"result": result, "server_address": server_address});
    } catch (error) {
      console.log(error);
      return Promise.reject(error);
    }
  ...
  websocketMessage(data) {
    switch(data.event) {
      ...
      case 'Shelly:CommandRequest':
        ...
        break;
      case 'Shelly:CommandRequest-timeout':
        return JSON.stringify({
          ...
          data: {
            cmd: data.command,
            params: { id: data.channel, [data.command_param]: data.command_value, timeout: data.timeout },
          }
        });

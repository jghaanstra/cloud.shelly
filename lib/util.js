...
class Util {
  ...
  sendCloudCommand(endpoint, address, token, device_id, body = null) {
    return new Promise((resolve, reject) => {
      ...
      fetch(address + endpoint, options)
  ...
  async getCloudLogin() {
    try {
      ...
      for (const driver of drivers) {
        ...
        if (devices.length > 0) {
          result = true;
          server_address = devices[0].getSetting('server_address');
          cloud_token = devices[0].getSetting('cloud_token');
          break;
        }
      }
      return Promise.resolve({"result": result, "server_address": server_address, "cloud_token": cloud_token});
    } catch (error) {
      console.log(error);
      return Promise.reject(error);
    }

...
class Util {
        ...
        .then(async res => {
          if (res.status === 200 ) {
            ...
            if (res.headers.get("www-authenticate") != undefined && (this.digest_retries[address] <= 2 || this.digest_retries[address] == undefined)) {
              ...
              return reject(res.status + ' '+ res.statusText +' - unauthenticated digest request. Retrying to create valid digest authentication.');
            } else {
              return reject(this.homey.__('util.401') +' Error message: '+ res.statusText + '. Too many retries or no authenticate header for digest authentication present');
            }
          } else {
  ...
  async getDeviceConfig(hostname, type = null) {
    if (type === null) {
      return Promise.resolve(Util.deviceConfig.find(r => r.hostname.includes(hostname)));
    } else {
      return Promise.resolve(Util.deviceConfig.find(r => r.type.includes(type)));
    }
  }

...
// node ./lib/helper.js non_cloud_drivers
// node ./lib/helper.js cloud_drivers
// node ./lib/helper.js non_battery_drivers

const main = async () => {
  const args = process.argv.slice(2);
  const drivers = appJson.drivers;
  if (args[0] === 'non_cloud_drivers') {
  ...
  } else if (args[0] === 'non_battery_drivers') {
    const filteredDrivers = drivers.filter((d) => !d.capabilities.includes('measure_battery'));
    const mappedDrivers = filteredDrivers.map((d) => d.id);

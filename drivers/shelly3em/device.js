...
class Shelly3EmDevice extends Homey.Device {
  ...
  onInit() {
    ...
    this.registerCapabilityListener('button.callbackevents', async () => {
      return await util.addCallbackEvents('/settings/relay/0?', callbacks, 'shelly3em', this.getData().id, this.getSetting('address'), this.getSetting('username'), this.getSetting('password'));
    });
  ...
  async onDeleted() {
    try {
      ...
      Homey.ManagerDrivers.getDriver('shelly3em').loadDevices();
      return;
    } catch (error) {

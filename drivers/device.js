...
class ShellyDevice extends Homey.Device {
  ...
  async bootSequence() {
    try {
      ...
      // gen1 + gen2: initially poll device status for powered devices
      if (!this.getStoreValue('battery')) {
        this.homey.setTimeout(() => {
          this.pollDevice();
        }, 2000 * this.getStoreValue('channel'));
      }

      // gen1 + gen2: start polling powered devices on regular interval
      if (this.getStoreValue('communication') === 'coap' && this.homey.settings.get('general_coap')) { // CoAP is disabled
        this.pollingFrequency = this.homey.settings.get('general_polling_frequency') * 1000 || 5000;
      }

      if (!this.getStoreValue('battery')) {
        this.pollingInterval = this.homey.setInterval(() => {
          this.pollDevice();
        }, (this.pollingFrequency + (1000 * this.getStoreValue('channel'))));
      }

      // TODO: remove this in the next big release, switched from server-client to client-server communication
      if (this.getStoreValue('communication') === 'websocket' && !this.getStoreValue('wsserver')) {
        const result = await this.util.setWsServer(this.getSetting('address'), this.getSetting('password'));
        if (result === 'OK') {
          this.setStoreValue('wsserver', true);
        }
      } 
        ...
        if (device) {
          if (device.hasCapability(capability)) {
            if (value !== device.getCapabilityValue(capability) && value !== null && value !== 'null' && value !== 'undefined' && value !== undefined) {
              return await device.setCapabilityValue(capability, value);
            }
          } else {
  ...
  async parseFullStatusUpdateGen2(result = {}) {
    try {
      ...
      // PRO 3EM instantaneous readings
      if (result.hasOwnProperty("em:0")) {
        ...
        /* meter_power_factor */
        this.parseCapabilityUpdate('a_pf', result["em:0"].a_pf, 0);
        this.parseCapabilityUpdate('b_pf', result["em:0"].b_pf, 1);
        this.parseCapabilityUpdate('c_pf', result["em:0"].c_pf, 2);
        ...
        if (result.method === 'NotifyStatus') { /* parse capability status updates */
          ...
          result.params.events.forEach(async (event) => {
            try {
              ...
              } else if (event.component.startsWith('emdata')) { // parse emdata events

                /* meter_power */
                this.parseCapabilityUpdate('energyCounter0', event.data.values[0][0], 0);
                this.parseCapabilityUpdate('energyCounter1', event.data.values[0][16], 1);
                this.parseCapabilityUpdate('energyCounter2', event.data.values[0][32], 2);

                /* meter_power_returned */
                this.parseCapabilityUpdate('energyReturned0', event.data.values[0][2], 0);
                this.parseCapabilityUpdate('energyReturned1', event.data.values[0][18], 1);
                this.parseCapabilityUpdate('energyReturned2', event.data.values[0][34], 2);
  ...
  async parseCapabilityUpdate(capability, value, channel = 0) {
    try {
      ...
      switch(capability) {
        ...
        case 'energyCounter2':
        case 'energyCounter3':
          if (this.getStoreValue('type') === 'SHEM' || this.getStoreValue('type') === 'SHEM-3' || this.getStoreValue('type') === 'SPEM-003CEBEU') {
  ...
  async updateDeviceConfig() {
    try {
      ...
      /* update device data from settings */
      if (this.getStoreValue('communication') === 'coap' && !this.getStoreValue('battery')) {
        if (result) {
      ...
      /* update device config from defined config */
